...
__device__ void dist_noflip(const float* point, RobotDimensions& dim, float* result_point)
// no angle flipping
{
...
__device__ float3 dist_noflipf3(float3& point, RobotDimensions& dim)
// no angle flipping
{
    // Coxa as the frame of reference without rotation
    float3 result = point;
...
__global__ void toVirdisUint_kernel(Matrixf table, unsigned char* pixels) {
    ...
    for (int i = index; i < table.height; i += stride) {
        ...
        const float color[3] = {VirdisData[x*3 + 0], VirdisData[x*3 + 1], VirdisData[x*3 + 2]};
