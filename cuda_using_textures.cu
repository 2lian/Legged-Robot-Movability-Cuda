...
bool reachability_from_coxa(float& x, float& y, RobotDimensions& dim) {
    ...
    // finding femur angle
    float required_angle_femur = atan2f(y, x);
    ...
    linnorm = fminf(
            norm3df(x - dim.positiv_saturated_femur[0], 0, y - dim.positiv_saturated_femur[1])
            ,
            norm3df(x - dim.negativ_saturated_femur[0], 0, y - dim.negativ_saturated_femur[1])
    );
...
// no angle flipping
{
    ...
    return reachability_from_coxa(result.x, result.z, dim);
}
...
// no angle flipping
{
    ...
    return reachability_from_coxa_texture(result.x, result.z, dim,
                                          reachabilitymap,
                                          text_min_x,
                                          text_max_x,
                                          text_min_y,
...
__global__ void toVirdisUint_kernel(Matrixf table, unsigned char* pixels, cudaTextureObject_t colormap) {
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;

    for (int i = index; i < table.height; i += stride) {

        const auto virdis_color = tex1D<float4>(colormap, table.elements[i]);

        pixels[i * 4 + 0] = (unsigned char) floorf(virdis_color.x * 255.f);
        pixels[i * 4 + 1] = (unsigned char) floorf(virdis_color.y * 255.f);
        pixels[i * 4 + 2] = (unsigned char) floorf(virdis_color.z * 255.f);
        pixels[i * 4 + 3] = (unsigned char) floorf(virdis_color.w * 255.f);
    }
}
...
__global__
void dist2virdis_pipelinef3(Arrayf3 arr, RobotDimensions dimensions, unsigned char* pixels, cudaTextureObject_t colormap)
{
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;
    for (int i = index; i < arr.length; i += stride) {
...
__global__
void reachability2img_tex_pipeline(Arrayf3 arr, RobotDimensions dimensions, unsigned char* pixels,
                                   cudaTextureObject_t reachabilitymap,
                                   float text_min_x,
                                   float text_max_x,
                                   float text_min_y,
                                   float text_max_y)
{
    ...
    for (int i = index; i < arr.length; i += stride) {
        bool result = reachability_texture(arr.elements[i], dimensions,
                                           reachabilitymap,
                                           text_min_x,
                                           text_max_x,
                                           text_min_y,
...
AutoEstimator::AutoEstimator(int pxWidth, int pxHeight, float scale) {
    ...
    define_virdis_as_texture();
    create_reachability_texture();
    error_check();
    setup_kernel();
    input_as_grid(scale);
    allocate_gpu_mem();
...
void AutoEstimator::define_virdis_as_texture(){
    const int width = 256;
    float4 f4_arr[width];

    for (int i = 0; i < width; i++) {
        f4_arr[i].x = VirdisColorMapCPU[i][0];
        f4_arr[i].y = VirdisColorMapCPU[i][1];
        f4_arr[i].z = VirdisColorMapCPU[i][2];
        f4_arr[i].w = 1;
    }
    ...
    cudaCreateTextureObject(&virdis1DTexture, &resDesc, &texDesc, nullptr);
...
void AutoEstimator::create_reachability_texture(){
    ...
    fill_mem_with_reachability_from_coxa<<<numBlocks, threadsPerBlock>>>(
            bool_arr,
            dimensions,
            width,
            height,
    ...
    cudaChannelFormatDesc channelDesc =
            cudaCreateChannelDesc(sizeof(unsigned char)*8,
                                  0,
                                  0,
                                  0,
    ...
    // Create texture object
    cudaCreateTextureObject(&reachabilityTexture, &resDesc, &texDesc, nullptr);
...
void AutoEstimator::convert_to_virdis(){
    ...
    toVirdisUint_kernel<<<numBlocks, blockSize >>>(result_norm_gpu,
                                                   virdisTexture_gpu, virdis1DTexture);
    cudaDeviceSynchronize();
    if (verbose) { std::cout << "Compute done" << std::endl;}
    error_check();
}
...
void AutoEstimator::reachability_to_img_pipeline_tex(){
    if (verbose) { std::cout << "dist_to_virdis_pipeline started" << std::endl;}
